...
(function($){
    ...
    $('#is_use_customer_address').change(function(){
        ...
        else
        {
            ...
            $('#receiver_post').val( $('#receiver_post').attr('data-from-origin') ).prop('disabled', false);
            $('#receiver_address').val( $('#receiver_address').attr('data-from-origin') ).prop('disabled', false);
        }
    });
	
	$('#edit_customer_receiver_address').click(function()
    {
		var $btn = $(this);
		$btn.button('loading');

		var customer_receiver_address_id = $('#customer_receiver_address_id').val();
		...
		$.post('/manager/e_store/customer/receiver_address/action/session/edit', data, function(resultObj)
        {
            showResultToastr({
                'resultObj': resultObj
            });
